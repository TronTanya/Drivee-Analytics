...
-- Каноническое имя датасета для NL→SQL и демо: все SELECT в продукте идут в «train» (VIEW = те же строки).
CREATE OR REPLACE VIEW public.incity_orders AS
...
-- Пароль для всех строк ниже: demo123 (passlib pbkdf2_sha256, как в app.core.security.hash_password).
INSERT INTO users (id, email, password_hash, is_demo_user, is_active, role_id) VALUES
('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1', 'admin@drivee.local', '$pbkdf2-sha256$29000$inHufe.dU6qVUsr5H6N0bg$L9yYlZzxwgLDrnZgog6Qwi1Mva6EXav/SxoUkK1IBH4', TRUE, TRUE, (SELECT id FROM roles WHERE role_key = 'admin' LIMIT 1)),
('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa2', 'manager@drivee.local', '$pbkdf2-sha256$29000$inHufe.dU6qVUsr5H6N0bg$L9yYlZzxwgLDrnZgog6Qwi1Mva6EXav/SxoUkK1IBH4', TRUE, TRUE, (SELECT id FROM roles WHERE role_key = 'manager' LIMIT 1)),
('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa3', 'marketer@drivee.local', '$pbkdf2-sha256$29000$inHufe.dU6qVUsr5H6N0bg$L9yYlZzxwgLDrnZgog6Qwi1Mva6EXav/SxoUkK1IBH4', TRUE, TRUE, (SELECT id FROM roles WHERE role_key = 'marketer' LIMIT 1)),
('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa4', 'executive@drivee.local', '$pbkdf2-sha256$29000$inHufe.dU6qVUsr5H6N0bg$L9yYlZzxwgLDrnZgog6Qwi1Mva6EXav/SxoUkK1IBH4', TRUE, TRUE, (SELECT id FROM roles WHERE role_key = 'executive' LIMIT 1))
...
('c0000000-0000-0000-0000-000000000001', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'orders_count', 'Количество заказов', 'Общее количество заказов', 'operations', 'metric', 'train', 'order_id', 'COUNT(*)', '{"unit":"rows"}', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'),
('c0000000-0000-0000-0000-000000000002', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'done_rides', 'Завершенные поездки', 'Заказы с driverdone_timestamp', 'operations', 'metric', 'train', 'driverdone_timestamp', 'COUNT(DISTINCT CASE WHEN driverdone_timestamp IS NOT NULL THEN order_id END)', '{"unit":"rows"}', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'),
('c0000000-0000-0000-0000-000000000003', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'client_cancellations', 'Отмены клиентом', 'Заказы с clientcancel_timestamp', 'operations', 'metric', 'train', 'clientcancel_timestamp', 'COUNT(CASE WHEN clientcancel_timestamp IS NOT NULL THEN 1 END)', '{"unit":"rows"}', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'),
('c0000000-0000-0000-0000-000000000004', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'avg_order_price', 'Средняя стоимость заказа', 'Среднее price_order_local', 'finance', 'metric', 'train', 'price_order_local', 'AVG(price_order_local)', '{"unit":"local_currency"}', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1')
