...
export default function NotebooksPage() {
  ...
  const renameMutation = useMutation({
    ...
    onError: () => {
      setListNotice("Не удалось сохранить имя. Проверьте авторизацию и workspace.");
    }
  });
  ...
  const handleNewScenario = async () => {
    ...
    const openLocalDemoScenario = () => {
      ...
      setListNotice("Создан локальный demo-сценарий (без записи в БД).");
    };
    const wid = workspaceQuery.data;
    if (!wid && !isDemoModeEnabled()) {
      setListNotice("Нужен workspace: войдите в систему.");
      return;
    }
    try {
        ...
        {!isLoading && notebooks.length === 0 ? (
          <div className="rounded-card border border-dashed border-border-subtle bg-surface-card px-6 py-12 text-center shadow-xs">
            ...
            <p className="mt-1 text-sm text-foreground-secondary">
              Создайте новый сценарий или откройте демо по ссылкам выше.
            </p>
            ...
            >
              {renameId === notebook.id ? (
                ...
                <Link
                  ...
                  className="min-w-0 flex-1 rounded-xl outline-none ring-brand-200 focus-visible:ring-2"
                >
                  <p className="text-lg font-medium text-slate-900">{notebook.title}</p>
                  <p className="mt-1 text-sm text-slate-600">
                    {notebook.notebook_status ?? "active"} ·{" "}
                    {formatDateUtc(notebook.updated_at ?? notebook.created_at)}
                  </p>
                </Link>
              )}
              {renameId !== notebook.id ? (
