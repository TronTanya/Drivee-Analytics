...
| `enterprise-state-empty` | Пустое состояние: пунктир, muted-фон |
| `enterprise-state-loading` | Контейнер загрузки + skeleton внутри `UiStateSurface` |
| `enterprise-state-error` | Ошибка: danger-soft, рамка |
| `enterprise-state-success` | Успех: emerald/success-soft (inline toast) |
| `enterprise-state-ambiguity` | Неоднозначность: лайм `border-l` + мягкий brand/amber фон |
| `enterprise-table-shell` | Обёртка desktop-таблицы в ноутбуке |

React-примитивы: [`components/ui/state-surface.tsx`](components/ui/state-surface.tsx), [`components/ui/badge.tsx`](components/ui/badge.tsx), [`components/ui/enterprise-input.ts`](components/ui/enterprise-input.ts).
